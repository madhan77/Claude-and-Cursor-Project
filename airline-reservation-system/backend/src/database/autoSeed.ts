...
export const runAutoSeed = async (): Promise<void> => {
  // Only run auto-seed if AUTO_SEED environment variable is set to true
  ...
  try {
    ...
    // Check if database is already seeded by looking for airlines
    const airlineCheck = await client.query('SELECT COUNT(*) FROM airlines');
    const airlineCount = parseInt(airlineCheck.rows[0].count);

    if (airlineCount > 0) {
      console.log(`✅ Database already has data (${airlineCount} airlines found). Skipping seed.\n`);
      return;
    }
    ...
    // Seed Flights with dates starting tomorrow
...
╚════════════════════════════════════════╝
    ...
    console.warn('⚠️  Database may not be available - skipping seed');
